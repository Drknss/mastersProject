...
void SceneRenderer::AIView() {
	
	if (!m_AiViewTex) glGenTextures(1, &m_AiViewTex);
	glBindTexture(GL_TEXTURE_2D, m_AiViewTex);
	...
	glGenFramebuffers(1, &m_AiFBO);

	glBindFramebuffer(GL_FRAMEBUFFER, m_AiFBO);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
		GL_TEXTURE_2D, m_AiViewTex, 0);

	GLuint status = glCheckFramebufferStatus(GL_FRAMEBUFFER_EXT);
	if (status != GL_FRAMEBUFFER_COMPLETE)
	{
		NCLERROR("Unable to create Ai Viewpoint Framebuffer! StatusCode: %x", status);
	}
...
bool SceneRenderer::ShadersLoad()
{
	...
	m_pSkyBoxShader = new Shader(SHADERDIR "SkyBox/skyboxVertex.glsl",
		SHADERDIR "SkyBox/skyboxFragment.glsl");
	if (!m_pSkyBoxShader->LinkProgram()) {
