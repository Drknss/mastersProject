			...
			case ENET_EVENT_TYPE_RECEIVE:
				if (evnt.packet->dataLength == sizeof(int)) {
					...
					if (temp == 0) {
						...
						ENetPacket* dest = enet_packet_create(&plnt, sizeof(int), 0);
						enet_host_broadcast(server.m_pNetwork, 0, dest);
					}
					if (temp == 2) {
						ENetPacket* dest = enet_packet_create(&plnt, sizeof(int), 0);
						enet_host_broadcast(server.m_pNetwork, 0, dest);
				...
				if (evnt.packet->dataLength == sizeof(Matrix3)) {
					Matrix3 temp;
					memcpy(&temp, evnt.packet->data, sizeof(Matrix3));

					Vector3 shipPos = Vector3(temp._11, temp._12, temp._13);
					Vector3 destPos = Vector3(temp._21, temp._22, temp._23);
					Vector3 direction = destPos - shipPos;
					direction.Normalise();
					direction = direction*60.f;

					ENetPacket* accelUpdate = enet_packet_create(&direction, sizeof(Vector3), 0);
					enet_host_broadcast(server.m_pNetwork, 0, accelUpdate);
